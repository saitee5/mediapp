...
return(
  <div className='min-h-screen flex font-sans bg-slate-100'>
   ...
   <div className='hidden lg:flex lg:w-[45%] bg-linear-to-b from-[#347d7b] via-[#3ca9a5] to-[#13545e] p-12 flex-col justify-between text-white relative overflow-hidden select-none'>
     ...
     {/*right*/}
     <div className='flex-1 flex flex-col justify-between p-8 sm:p-12 lg:p-20 bg-white overflow-y-auto'>
        ...
        {!isRegistering ? (
          <>
           {/* Header */}
              ...
              <p className="text-center text-sm font-medium text-slate-500">
                ...
                <button onClick={() => setIsRegistering(true)} type="button" className="font-semibold text-[#007e7a] hover:underline cursor-pointer">
          ...
          <>
            <div className="space-y-2 text-center lg:text-left">
              ...
              <p className="text-sm text-slate-500 font-medium">
                Set up your clinic to start generating AI medical documents.
              </p>
            </div>

            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsRegistering(false); }}>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Clinic Name</label>
                <input type="text" required value={regClinicName} onChange={(e)=>setRegClinicName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all font-medium" placeholder="E.g. Ezovion Clinic" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Doctor Name & Speciality</label>
                <input type="text" required value={regDoctorName} onChange={(e)=>setRegDoctorName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all font-medium" placeholder="Dr. John Doe, Cardiology" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Official Email</label>
                <input type="email" required value={regEmail} onChange={(e)=>setRegEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all font-medium" placeholder="doctor@clinic.com" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Clinic Address</label>
                <input type="text" required value={regAddress} onChange={(e)=>setRegAddress(e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all font-medium" placeholder="123 Health St." />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Clinic Logo</label>
                <input type="file" accept="image/*" className="w-full bg-slate-50 border border-slate-200 text-slate-800 px-4 py-2 rounded-xl text-sm file:mr-4 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100 transition-all cursor-pointer" />
              </div>
              
              <button type="submit" className="w-full bg-[#007e7a] hover:bg-teal-700 text-white font-bold py-3 px-4 rounded-xl text-sm transition-colors shadow-sm focus:outline-none cursor-pointer mt-2">
            ...
            <p className="text-center text-sm font-medium text-slate-500">
              Already registered?{' '}
              <button onClick={() => setIsRegistering(false)} type="button" className="font-semibold text-[#007e7a] hover:underline cursor-pointer">
                Back to login
              </button>
            </p>
          </>
